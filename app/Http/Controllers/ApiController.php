...
class ApiController extends BaseController
{
    ...
    function auth(Request $request){
        try{
            ...
            $key = date("ymd");
            $key = $key + 123456;
            $key = $key.gen_rand_number(3);
            $appkey = $type.$key;
            $appsecret = sha1($appkey.gen_rand_string(10));
            $status = 1;
            $tpname = $tpnames[$type];
            $sql = "insert into vault_sys_auth (appkey,appsecret,status,tpname) values ('$appkey','$appsecret','$status','$tpname')";

            $id = DB::connection('mysql_230')->insert($sql);
            if(true != $id) {
                echo "系统内部错误!";
                return;
            }
